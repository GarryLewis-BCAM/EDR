...
class EDRLogger:
    def __init__(self, name: str, log_dir: str, nas_log_dir: Optional[str] = None,
        ...
        # Try to create NAS log dir, but don't fail if unavailable
        ...
        if self.nas_log_dir:
            try:
                ...
                # NAS not available - will log later when logger is ready
                self.nas_available = False
                self.nas_error = str(e)
    ...
    def sync_to_nas(self):
        ...
        if not self.nas_log_dir or not getattr(self, 'nas_available', False):
            return False
