...
class FileEvent:
    ...
    def validate(self):
        """Validate all fields"""
        valid_types = ['created', 'modified', 'deleted', 'moved']
        if self.event_type not in valid_types:
            raise ValidationError(f"Invalid event_type: {self.event_type}. Must be one of {valid_types}")
        if not self.path or not isinstance(self.path, str):
            raise ValidationError("File path must be non-empty string")
...
class EDRDatabase:
    ...
    def __init__(self, db_path: str, nas_backup_path: Optional[str] = None):
        ...
        # Try to create NAS backup path, but don't fail if unavailable
        ...
        if self.nas_backup_path:
            try:
                ...
                logger.warning(f"NAS backup path unavailable: {e}. Database backups disabled.")
                self.nas_available = False
    ...
    def _init_schema(self):
        ...
        with self._transaction() as conn:
            ...
            CREATE TABLE IF NOT EXISTS ups_events (
                ...
                replace_battery INTEGER DEFAULT 0,
                connected INTEGER DEFAULT 1,
                device_name TEXT,
                alert_triggered INTEGER DEFAULT 0
            );
            CREATE INDEX IF NOT EXISTS idx_ups_timestamp ON ups_events(timestamp);
            CREATE INDEX IF NOT EXISTS idx_ups_alerts ON ups_events(alert_triggered, timestamp);
    ...
    def insert_file_event(self, event: Dict[str, Any]) -> Optional[int]:
        """
        Insert file system event with full validation
        
        Args:
            event: Dictionary containing file event data
        ...
        """
        try:
            # Validate and normalize using FileEvent dataclass
            file_event = FileEvent(**event)
            
            with self._transaction() as conn:
                cursor = conn.execute('''
                    INSERT INTO file_events 
                    (timestamp, event_type, path, process_name, is_suspicious)
                    VALUES (?, ?, ?, ?, ?)
                ''', (
                    file_event.timestamp,
                    file_event.event_type,
                    file_event.path[:2048],  # Limit path length
                    file_event.process_name[:255] if file_event.process_name else None,
                    1 if file_event.is_suspicious else 0
                ))
                return cursor.lastrowid
        except ValidationError:
            raise  # Re-raise validation errors
        except sqlite3.Error as e:
            logger.error(f"Failed to insert file event: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error inserting file event: {e}", exc_info=True)
            return None
    ...
    def insert_ups_event(self, event: Dict[str, Any]) -> Optional[int]:
        """
        Insert UPS power event
        
        Args:
            event: Dictionary containing UPS event data
        ...
        try:
            with self._transaction() as conn:
                cursor = conn.execute('''
                    ...
                    (timestamp, event_type, status, battery_charge, runtime_minutes, load_percent,
                     on_battery, low_battery, replace_battery, connected, device_name, alert_triggered)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    event.get('timestamp'),
                    event.get('event_type'),
                    event.get('status'),
                    event.get('battery_charge'),
                    event.get('runtime_minutes'),
                    event.get('load_percent'),
                    1 if event.get('on_battery') else 0,
                    1 if event.get('low_battery') else 0,
                    1 if event.get('replace_battery') else 0,
                    1 if event.get('connected', True) else 0,
                    event.get('device_name'),
                    1 if event.get('alert_triggered') else 0
                ))
                return cursor.lastrowid
        except sqlite3.Error as e:
            logger.error(f"Failed to insert UPS event: {e}")
            return None
        except Exception as e:
            logger.error(f"Unexpected error inserting UPS event: {e}", exc_info=True)
            return None
    ...
    def backup_to_nas(self) -> bool:
        ...
        if not self.nas_backup_path:
            logger.debug("NAS backup path not configured")
            return False
    ...
    def _auto_checkpoint(self):
        """Automatically checkpoint WAL if interval elapsed"""
        try:
    ...
    def checkpoint_wal(self):
        ...
        try:
            ...
            if result:
                logger.debug(f"WAL checkpoint: {result[0]} frames written, {result[1]} frames checkpointed")
        except Exception as e:
