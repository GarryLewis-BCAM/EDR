...
- Auto-clears Python cache on startup
"""
import sys
import time
import signal
...
class EDRCollectorV2:
    ...
    def __init__(self, config_path: str):
        ...
        # Warn if NAS is unavailable
        if not getattr(self.logger, 'nas_available', True):
            print(f"⚠️  WARNING: NAS storage unavailable: {getattr(self.logger, 'nas_error', 'Unknown')}")
            print(f"⚠️  Continuing with local logging only")
        ...
        if not getattr(self.logger, 'nas_available', True):
            self.logger.warning(
                f"NAS storage unavailable: {getattr(self.logger, 'nas_error', 'Unknown')}. Using local logs only."
            )
        ...
        try:
            self.ups_monitor = UPSMonitor(self.config, self.logger, self.db)
            self.logger.info("✓ UPS monitor initialized")
        except Exception as e:
            self.logger.error(f"UPS monitor initialization failed: {e}")
        ...
        try:
            ...
            if self.network_tracker and self.collection_count % 3 == 0:
                try:
                    ...
                    # Network collection errors are not fatal - just log and continue
                    self.logger.warning(f"Network collection failed (non-fatal): {e}")
            
            # Collect UPS data (matches poll_interval, typically every cycle at 30s)
            if self.ups_monitor and self.config['collection'].get('ups_monitor', {}).get('enabled', False):
                try:
                    ups_events = self.ups_monitor.collect()
                    if ups_events:
                        events_collected += len(ups_events)
                        # Send alerts for power events
                        for ups_event in ups_events:
                            if ups_event.get('type') in ['ups_disconnected', 'ups_reconnected', 'power_failure', 'battery_critical', 'power_restored']:
                                self._send_ups_alert(ups_event)
                except Exception as e:
                    # UPS collection errors are not fatal
                    self.logger.warning(f"UPS collection failed (non-fatal): {e}")
    ...
    def _health_check(self):
        ...
        try:
            ...
            # Check NAS connectivity - verify actual share mounts
            if self.config['nas']['enabled']:
                import os
                ...
                # Only report issue if less than 2 of 3 shares are mounted
                if len(mounted_shares) < 2:
                    health_issues.append(f"NAS connectivity issue: only {len(mounted_shares)}/3 shares mounted")
    ...
    def _send_ups_alert(self, ups_event: Dict):
        """Send UPS power event alert via configured channels"""
        if not self.alerter:
            self.logger.warning("No alerter configured for UPS events")
            return
        
        try:
            # Handle priority - can be string or enum
            priority = ups_event.get('priority', 'MEDIUM')
            if isinstance(priority, str):
                ...
                priority_enum = priority_map.get(priority.upper(), AlertPriority.MEDIUM)
                severity = priority.lower()
            else:
                priority_enum = priority
                severity = priority.value.lower()
            
            self.logger.info(f"Creating UPS alert: title={ups_event.get('title')}, severity={severity}, priority={priority_enum}")
            
            alert = Alert(
                title=ups_event.get('title', 'UPS Event'),
                message=ups_event.get('message', 'UPS status changed'),
                priority=priority_enum,
                severity=severity,
                source='UPSMonitor',
                details=ups_event.get('status', {})
            )
            
            self.logger.info(f"Sending UPS alert to alerter...")
            self.alerter.send_alert(alert)
            self.logger.info(f"✅ UPS alert sent successfully: {ups_event.get('type')}")
        except Exception as e:
            self.logger.error(f"Failed to send UPS alert: {e}", exc_info=True)
